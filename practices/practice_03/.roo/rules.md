...
## Память проекта (MCP Memory)

- При вопросах об истории проекта, прошлых решениях, договорённостях или контексте — **первым делом вызывай `search_nodes` из MCP-сервера `memory`**, не читай файлы
- После каждой значимой договорённости или архитектурного решения — сохраняй в memory через `create_entities` / `add_observations`
- Файлы (`AGENTS.md`, `docs/`) — источник актуального кода, memory — источник истории и решений
